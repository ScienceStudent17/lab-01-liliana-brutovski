...
There are 1846 rows and 3 variables. The variable names are **dataset**, **x**, and **y**.
...
dino_data %>%
  summarize(r = cor(x, y))
```
The r value for the star dataset is very similar to the r of the dino dataset, and they are within two thousandths of each other. Both r values are extremely low, which indicates little to no correlation between x and y.
...
dino_data %>%
  summarize(r = cor(x, y))
```
The r value for the circle dataset is similar to the r value for the dino dataset. However, the circle r value shows greater correlation than the dino r value.
